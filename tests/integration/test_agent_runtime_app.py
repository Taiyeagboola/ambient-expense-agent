...
async def test_agent_stream_query(agent_app: AgentEngineApp) -> None:
    ...
    Tests that the agent returns valid streaming responses for a valid expense event.
    """
    # Send a valid auto-approvable expense event JSON string
    message = '{"data": {"amount": 50.0, "submitter": "Alice"}}'
    events = []
    async for event in agent_app.async_stream_query(message=message, user_id="test"):
        events.append(event)
    assert len(events) > 0, "Expected at least one chunk in response"

    # Check for valid approval content in the response
    has_approval_content = False
    for event in events:
        ...
        if (
            ...
            and output.get("status") == "approved"
            and output.get("auto_approved") is True
        ):
            has_approval_content = True
            break

    assert has_approval_content, "Expected an event indicating auto-approval"
